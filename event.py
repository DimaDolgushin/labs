    ...
    def allow_button_clicked(self) -> None:
        if self.text_line_edit.text() == '':
            QtWidgets.QMessageBox(icon=QtWidgets.QMessageBox.Icon.Critical,
                                  ...
                                  parent=self)
            return
        self.list_widget.addItem(QtWidgets.QListWidgetItem(
            f'{self.calendar.selectedDate().toPython()} {self.time_edit.text()} - {self.text_line_edit.text()}'))

    def closeEvent(self, event: QtGui.QCloseEvent) -> None:
        all_items = self.list_widget.findItems("", QtCore.Qt.MatchFlag.MatchContains)
        all_items_text = str([item.text() for item in all_items]).replace('[', "").replace(']', "").replace("\'",
                                                                                                            '').replace(
            ', ', '\n')
